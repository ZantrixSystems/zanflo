  ...
  return (
    <div className="section-field-row">
      <input
        ...
        placeholder="Field key (e.g. opening_until)"
        value={field.key}
        onChange={(e) => onChange(index, { ...field, key: slugify(e.target.value) })}
      />
      <input
        type="text"
        className="form-input"
...
function SectionForm({ initial, onSave, onCancel }) {
  ...
  const [fields, setFields]         = useState(
    Array.isArray(initial?.fields) ? initial.fields : []
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="section-form">
      ...
      <div className="form-group">
        ...
        <button type="button" className="btn btn-secondary btn-sm" onClick={addField} style={{ marginTop: 8 }}>
          + Add field
        </button>
      </div>
      ...
      <div className="form-actions">
        ...
        <button type="submit" className="btn btn-primary" disabled={saving}>
          {saving ? 'Saving…' : initial ? 'Save changes' : 'Create section'}
        </button>
      </div>
    </form>
  );
  ...
  return (
    <AdminLayout
      ...
      breadcrumbs={[
        ...
        { label: 'Licence sections' },
      ]}
    >
      <div className="queue-page-header">
        <h1 className="queue-page-title">Licence sections</h1>
        <p className="queue-page-subtitle">
          Define which licence sections applicants can select when creating a premises case.
          Each section can include structured questions.
        </p>
      </div>
      ...
      {!creating && (
        <button
          ...
          onClick={() => { setCreating(true); setEditingId(null); setNotice(''); setError(''); }}
      ...
      {/* Create form */}
      {creating && (
        <div className="case-actions-card" style={{ marginBottom: '1.5rem' }}>
          <div className="case-actions-title">New section</div>
          <SectionForm
            onSave={handleCreate}
      ...
      {loading ? (
        ...
        <div className="queue-empty">
          <div className="queue-empty-title">No sections defined yet</div>
          <p className="queue-empty-hint">Create your first section above.</p>
        </div>
      ) : (
        <div className="sections-list">
          {sections.map((sec) => (
            <div key={sec.id} className={`section-card${sec.is_enabled ? '' : ' section-card-disabled'}`}>
              {editingId === sec.id ? (
                <>
                  <div className="case-actions-title">Edit: {sec.name}</div>
                  <SectionForm
                    initial={sec}
                    onSave={(body) => handleUpdate(sec.id, body)}
                ...
                <>
                  <div className="section-card-header">
                    ...
                    <div className="section-card-actions">
                      <button type="button" className="btn btn-secondary btn-sm" onClick={() => { setEditingId(sec.id); setCreating(false); setNotice(''); setError(''); }}>
                        Edit
                      </button>
                      <button type="button" className="btn btn-secondary btn-sm" onClick={() => handleToggle(sec)}>
                        {sec.is_enabled ? 'Disable' : 'Enable'}
                      </button>
                      <button type="button" className="btn btn-danger btn-sm" onClick={() => handleDelete(sec.id)}>
                        Delete
                      </button>
                    </div>
                  </div>
                  ...
                  {Array.isArray(sec.fields) && sec.fields.length > 0 && (
                    <div className="section-card-fields">
                      <div className="section-card-fields-label">

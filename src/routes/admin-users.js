...
async function createUser(request, env) {
  ...
  if (!user) {
    ...
    const usernameClash = username ? await sql`
...
async function updateUser(request, env, userId) {
  ...
  if (!role && !fullName && !Object.prototype.hasOwnProperty.call(body, 'username')) {
  ...
  if (Object.prototype.hasOwnProperty.call(body, 'username') && username) {
  ...
  if (fullName || Object.prototype.hasOwnProperty.call(body, 'username')) {
    await sql`
      UPDATE users
      SET
        full_name = COALESCE(${fullName}, full_name),
        username = CASE
          WHEN ${Object.prototype.hasOwnProperty.call(body, 'username')} THEN ${username}
          ELSE username
        END
      WHERE id = ${userId}
    `;
